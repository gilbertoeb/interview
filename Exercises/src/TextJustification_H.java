 ...
 *   -Create an empty list res to store the justified lines.
 * <p>
 * Iterate through the words:
 *   -Use a while loop to iterate through the words array with an index i.
 * <p>
 * Determine the range of words for the current line:
 *    <p>-Initialize j to i + 1 and len to the length of the word at index i.
 *    <p>-Use a nested while loop to find the range of words that can fit in the current
 *   line without exceeding maxWidth. Update len to include the length of the next
 *   word and the spaces between words.
 * <p>
 * Build the current line:
 *    <p>-Create a StringBuilder sb to build the current line.
 *    <p>-Calculate the number of spaces needed to justify the line:
 *      <p>->If it is the last line or the line contains only one word, left-justify the
 *       line by appending words followed by a single space and then padding with
 *       extra spaces at the end.
 *      <p>->Otherwise, distribute the spaces as evenly as possible between the words.
 *       Calculate the number of spaces (spaces) and the remainder (r) to distribute
 *       extra spaces to the left slots.
 * <p>
